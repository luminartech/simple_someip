...
pub type DispatchFn = fn(service_id: u16, method_id: u16, payload: &[u8], e2e_status: u8);
...
static SUBS_STORAGE: StaticSubscriptionStorage =
...
fn find_offer(service_id: u16, instance_id: u16, event_group_id: u16) -> Option<&'static OfferEntry> {
    offers().iter().find(|o| {
...
fn platform() -> Platform<'static, RX_SLOTS, RX_CAP> {
    ...
    let send: SendFn = unsafe { core::mem::transmute::<usize, SendFn>(SEND_FN.load(Ordering::Acquire)) };
    let now: NowMsFn = unsafe { core::mem::transmute::<usize, NowMsFn>(NOW_FN.load(Ordering::Acquire)) };
    Platform {
        send,
        now_ms: now,
        mailbox,
...
fn dispatch(service_id: u16, method_id: u16, payload: &[u8], e2e_status: u8) {
    ...
    f(service_id, method_id, payload, e2e_status);
}
...
fn build_server() -> bool {
    ...
    unsafe {
        ...
        (*core::ptr::addr_of_mut!(PUBLISHER)).write(EventPublisher::new(subs_handle, unicast_ref, e2e));
    }
    ...
    for entry in o {
        config = config.with_accepted_offer(entry.service_id, entry.instance_id, entry.event_group_id);
    }
    ...
    let storage = unsafe {
        ServerStorage {
            ...
            non_sd_observer: Some(dispatch as crate::server::NonSdRequestCallback),
        }
    };
...
async fn someip_task() {
    ...
    let recv = server.recv_only_future(&mut bufs.unicast, &mut bufs.sd);
...
pub fn init(config: RuntimeConfig) -> i32 {
    ...
    SD_PORT.store(
        if config.sd_port == 0 { DEFAULT_SD_PORT } else { config.sd_port },
        Ordering::Release,
    );
    SD_MCAST.store(
        if config.sd_mcast == 0 { DEFAULT_SD_MCAST } else { config.sd_mcast },
        Ordering::Release,
    );
    ...
    bind(SD_PORT.load(Ordering::Acquire), true, SD_MCAST.load(Ordering::Acquire));
...
pub unsafe fn publish(
    ...
    publish_notification(
        ...
        |datagram, dst| {
            ...
            (plat.send)(src_port, datagram.as_ptr(), datagram.len(), dst_addr, dst.port());
        },
    )
}
...
pub fn deinit() {
    ...
    if let Ok(total) = build_multi_stop_offer_service_datagram::<MAX_OFFERS>(scratch, &reqs[..n], session) {
