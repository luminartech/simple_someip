...
impl EventPublisher {
    ...
    /// SINGLE subscriber, addressed by endpoint.
    ///
    /// Unlike [`Self::publish_raw_event`], which fans out to every subscriber of
    /// the event group, this targets one — the caller chooses which subscriber
    /// (by its subscribed endpoint) receives the event. This is what enables
    /// per-recipient delivery when several receivers share the same
    /// `(service_id, instance_id, event_group_id)` key but bind distinct
    /// endpoints (e.g. multiple sensors on one subnet that share a fixed
    /// instance id).
    ///
    /// Returns `1` if `target` is a current subscriber of the event group and
    /// the datagram was sent, `0` if `target` is not subscribed (mirroring the
    /// fan-out path's "0 == nothing sent" semantics, per target). A send error
    /// to a subscribed target is logged and reported as `0`.
    ///
    /// # Errors
    ///
    /// Returns an error if the SOME/IP header fails to serialize.
    ...
    pub async fn publish_raw_event_to(
        ...
        match self.socket.send_to(&buffer, target).await {
            Ok(_) => Ok(1),
            Err(e) => {
                tracing::error!("Failed to send raw event to {}: {:?}", target, e);
                Ok(0)
            }
        }
    }
...
mod tests {
    ...
    async fn test_publish_raw_event_to_targets_one_subscriber() {
        ...
        let addr_a = match rx_a.local_addr().unwrap() {
            std::net::SocketAddr::V4(a) => a,
            _ => panic!("expected v4"),
        };
        let addr_b = match rx_b.local_addr().unwrap() {
            std::net::SocketAddr::V4(a) => a,
            _ => panic!("expected v4"),
        };
        {
            let mut mgr = subscriptions.write().await;
            mgr.subscribe(0x5B, 2, 0x01, addr_a);
            mgr.subscribe(0x5B, 2, 0x01, addr_b);
        }
    ...
    async fn test_subscriber_addresses_lists_all_under_key() {
        ...
        {
            let mut mgr = subscriptions.write().await;
            mgr.subscribe(0x5B, 2, 0x01, a);
            mgr.subscribe(0x5B, 2, 0x01, b);
        }
        let (publisher, _) = make_publisher(subscriptions).await;
