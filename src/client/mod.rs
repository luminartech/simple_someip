...
/// Slot capacity (channel depth) of the per-socket `SendMessage` and
/// `ReceivedMessage` MPSC channels held by every [`SocketManager`].
///
/// Each static channel slot stores its ring buffer inline as `[T; CAP]`,
/// so this value is the dominant term in the bare-metal channel-pool
/// footprint: the `SendMessage` / `ReceivedMessage` payload types embed a
/// `HeaplessPayload` (hundreds of bytes), and there are two such pools per
/// bound socket. Lowered from 16 to 4 to shrink those pools on DSPR0/LMU;
/// the socket loop only needs to absorb a brief burst before backpressure
/// engages, so reducing the depth has no semantic effect for a low-rate
/// embedded workload (see
/// [`SocketManager::bind_with_transport_unspawned`]).
///
/// Bare-metal [`define_static_channels!`](crate::define_static_channels)
/// consumers MUST declare their `SendMessage` / `ReceivedMessage` bounded
/// channels at this exact slot cap, or the
/// `BoundedPooled<C, { CLIENT_SOCKET_CHANNEL_CAP }>` bounds throughout the
/// client will not be satisfied.
pub const CLIENT_SOCKET_CHANNEL_CAP: usize = 4;
#[cfg(feature = "client-tokio")]
...
/// | `bounded` | `(SendMessage<P, C>, CLIENT_SOCKET_CHANNEL_CAP)` | per-pool default |
/// | `bounded` | `(Result<ReceivedMessage<P>, client::Error>, CLIENT_SOCKET_CHANNEL_CAP)` | per-pool default |
...
where
    ...
    SendMessage<P, Self>: BoundedPooled<Self, { CLIENT_SOCKET_CHANNEL_CAP }>,
    Result<ReceivedMessage<P>, Error>: BoundedPooled<Self, { CLIENT_SOCKET_CHANNEL_CAP }>,
...
where
    ...
    SendMessage<P, C>: BoundedPooled<C, { CLIENT_SOCKET_CHANNEL_CAP }>,
    Result<ReceivedMessage<P>, Error>: BoundedPooled<C, { CLIENT_SOCKET_CHANNEL_CAP }>,
...
where
    ...
    SendMessage<MessageDefinitions, C>: BoundedPooled<C, { CLIENT_SOCKET_CHANNEL_CAP }>,
    Result<ReceivedMessage<MessageDefinitions>, Error>:
        BoundedPooled<C, { CLIENT_SOCKET_CHANNEL_CAP }>,
